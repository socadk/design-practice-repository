...
5. *Map candidate endpoints to existing or new backend systems, yielding a [Refined Endpoint List](../artifact-templates/SDPR-RefinedEndpointList.md)*, and *decompose monolithic backends into (micro-)services* (@Newman:2015) to promote flexibility and scalability if these are desired qualities and your software engineering (and operations) toolbox is rich and mature enough. [*Refactor*](https://www.ifs.hsr.ch/Architectural-Refactoring-for.12044.0.html?&L=4) (@Zimmermann:2017) the Candidate Endpoint List and the preliminary architecture from the previous steps along the way (along with any tentatively added remote facades and DTOs). Document and justify these architectural decisions too. 
...
In this DPR repository, these seven top-level steps and activities are supported by one or more [*artifact templates*](../artifact-templates): 
...
5. The [Refined Endpoint List](../artifact-templates/SDPR-RefinedEndpointList.md) then is more precise and assertive. 
